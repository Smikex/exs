...
if(!function_exists('is_admin_newurl')){
	function is_admin_newurl($item){
		$item = pn_string($item);
		$new_item = delsimbol(replace_cyr($item));
	...
	function prepare_form_fileds($items, $filter, $prefix){
		...
		if(is_array($items)){
			foreach($items as $name => $data){
				...
				$line = '
				<div class="form_field_line '. $prefix .'_line type_'. $type .' '. $tooltip_class  .'">';
					if($title){
						$line .= '<div class="form_field_label '. $prefix .'_label"><label for="'. $prefix .'_id-'. $name .'"><span class="form_field_label_ins">'. $title .''. $req_html .':'. $tooltip_span .'</span></label></div>';
					}
					$line .= '
				...
				if($type == 'text'){
					...
					$line .= '
					<input type="text" id="'. $prefix .'_id-'. $name .'" class="'. $prefix .'_input '. $class .'" '. $disabled .' placeholder="'. $placeholder .'" '. $not_auto_html .' name="'. $name .'" value="'. $value .'" />						
					';
