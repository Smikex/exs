	...
	function pn_admin_work_options($options, $template){
		...
		foreach($options as $option){
			...
			if($view == 'h3'){
				...
				$sel_options = is_isset($option,'options');
				pn_select_search($title, $name, $sel_options, $default, $class, $template);			
...
if(!function_exists('pn_select_search')){
	function pn_select_search($title, $name='', $options=array(), $default='', $class='', $template=''){
		$options = (array)$options;
		$temp = '';
		$template = pn_set_option_template($template);			
		$temp .= $template['before'];
		...
		$temp .= '</select><input type="search" name="" class="js_select_search premium_input" placeholder="'. __('Search...','premium') .'" value="" /></div>';			
		$temp .= $template['after_content'];
		$temp .= $template['after'];
		$temp = str_replace("[class]", $class, $temp);
		echo $temp;
