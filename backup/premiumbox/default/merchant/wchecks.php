    ...
    function column_default($item, $column_name){
		...
		if($column_name == 'descr'){
			...
			if($item['status'] == 'active'){
				$settings_html .=' | <a href="'. admin_url('admin.php?page=pn_data_wchecks&m_id='.$item['name']) .'" target="_blank">'. __('Settings','pn') .'</a> ';
			}
			
			$html = '
				<div>'. pn_strip_input(ctv_ml($item['description'])) .'</div>
				<div class="active second plugin-version-author-uri">'. __('Version','pn') .': '. pn_strip_input($item['version']) . $settings_html . apply_filters('wchecks_settingtext_'. $item['name'],'') .'</div>
			';
