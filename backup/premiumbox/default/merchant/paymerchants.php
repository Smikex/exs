...
/* настройки */
add_action('pn_adminpage_content_pn_paymerchants','def_pn_admin_content_pn_paymerchants');
function def_pn_admin_content_pn_paymerchants(){

	if(class_exists('trev_paymerchants_List_Table')){
		$Table = new trev_paymerchants_List_Table();
		$Table->prepare_items();
...
$(function(){
	...
	$('select.merchant_change').change(function(){ 
        ...
        $.ajax({
			type: "POST",
			url: "<?php pn_the_link_post('paymerchants_settings_save'); ?>",
			data: param,
...
add_action('premium_action_paymerchants_settings_save', 'pn_premium_action_paymerchants_settings_save');
function pn_premium_action_paymerchants_settings_save(){
global $wpdb;
	...
	if(current_user_can('administrator') or current_user_can('pn_merchants')){
		...
		$merchants = apply_filters('paymerchant_settings_save', $merchants, $id, $wid);
		
		update_option('paymerchants', $merchants);
	...
	if(isset($_POST['id']) and is_array($_POST['id'])){
		...
		if($action == 'active'){
			...
			foreach($_POST['id'] as $id){
				...
				if($id and !isset($pn_extended['paymerchants'][$id])){
					$pn_extended['paymerchants'][$id] = $id;
					pn_include_extanded('paymerchants', $id);
					do_action('pn_paymerchants_active_'.$id);
				}	
			}
		...
		if($action == 'deactive'){
			...
			foreach($_POST['id'] as $id){
				$id = is_extension_name($id);
				if($id and isset($pn_extended['paymerchants'][$id])){
					unset($pn_extended['paymerchants'][$id]);
					do_action('pn_paymerchants_deactive_'.$id);
...
global $wpdb;	
	...
	if($id){
		...
		if(!isset($pn_extended['paymerchants'][$id])){
			$pn_extended['paymerchants'][$id] = $id;
...
class trev_paymerchants_List_Table extends WP_List_Table { 
    ...
    function column_default($item, $column_name){
		...
		if($column_name == 'descr'){
			...
			if($item['status'] == 'active'){
				$settings_html .=' | <a href="'. admin_url('admin.php?page=pn_data_paymerchants&m_id='.$item['name']) .'" target="_blank">'. __('Settings','pn') .'</a> ';
			}
			
			$html = '
				<div>'. pn_strip_input(ctv_ml($item['description'])) .'</div>
				<div class="active second plugin-version-author-uri">'. __('Version','pn') .': '. pn_strip_input($item['version']) . $settings_html . apply_filters('paymerchants_settingtext_'. $item['name'],'') .'</div>
			';
			...
			if($item['status'] == 'active'){
				...
				$default = is_enable_paymerchant($item['name']);
    ...
    function column_title($item){
		...
		if($item['status'] == 'active'){
			$actions['deactive']  = '<a href="'. pn_link_post('pn_paymerchants_deactivate'). '&key='. $item['name'] .'&_wp_http_referer=' . urlencode($_SERVER['REQUEST_URI']) .'">'. __('Deactivate','pn') .'</a>';
		} else {
			$actions['active']  = '<a href="'. pn_link_post('pn_paymerchants_activate'). '&key='. $item['name'] .'&_wp_http_referer=' . urlencode($_SERVER['REQUEST_URI']) .'">'. __('Activate','pn') .'</a>';
		}
    ...
    function get_columns(){
        $columns = array(
			...
			'title'     => __('Title','pn'),
			'name' => __('Name','pn'),
			'descr'     => __('Automatic payout description','pn'),
			'status'     => __('Status','pn'),
        );
    ...
    function prepare_items() {
        ...
        $per_page = $this->get_items_per_page('trev_paymerchants_per_page', 50);
        $current_page = $this->get_pagenum();
        
        $this->_column_headers = $this->get_column_info();
		$offset = ($current_page-1)*$per_page;

		$list = pn_list_extended('paymerchants');
		$ndata = array();
...
add_action('premium_screen_pn_paymerchants','my_myscreen_pn_paymerchants');
function my_myscreen_pn_paymerchants() {
    $args = array(
        'label' => __('Display','pn'),
        'default' => 50,
        'option' => 'trev_paymerchants_per_page'
    );
    add_screen_option('per_page', $args );	
	if(class_exists('trev_paymerchants_List_Table')){
		new trev_paymerchants_List_Table;
	}
}