...
function def_pay_purse_merchant($purse, $m_data, $m_id){
...
global $wpdb;	
	...
	$bid_sum = is_isset($params, 'bid_sum');
	...
	if($id and $status){
		...
		if(isset($item->id)){
			...
			if($bid_status != $status){
				...
				$sum = is_my_money($sum, 12);
				$bid_sum = is_my_money($bid_sum, 12);
				if($bid_sum <= 0){
					$bid_sum = is_my_money($item->summ1_dc, 12);
				}				
...
global $wpdb;	
	...
	$array['sum'] = $array['pay_com'] = $array['com_pers'] = $array['com_sum'] = $array['nscom'] = $array['pay_sum'] = 0;
