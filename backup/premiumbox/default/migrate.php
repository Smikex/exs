...
<div class="premium_body">
	<table class="premium_standart_table">
		<?php
		pn_h3(sprintf(__('Migration (if version is lesser than %s)','pn'),'1.3'), '');
		...
		<tr>
			<td>		
				<input name="submit" type="submit" class="button pn_prbar" data-count-url="<?php pn_the_link_ajax('migrate_step_count'); ?>&step=3_<?php echo $r; ?>" data-title="<?php printf(__('Step %s','pn'),$r); ?>" value="<?php printf(__('Step %s','pn'),$r); ?>" />	
				<input name="submit" type="submit" class="button pn_prbar" data-count-url="<?php pn_the_link_ajax('migrate_step_count'); ?>&step=3_<?php echo $r; ?>&tech=1" data-title="<?php printf(__('Step %s','pn'),$r); ?>" value="<?php printf(__('Technical step %s','pn'),$r); ?>" />		
			</td>
		</tr>
		<?php 
		} 
...
global $wpdb;	
	...
	if(current_user_can('administrator')){
		...
		if(!$tech){
			...
			if($step == '3_1'){
				$count = $wpdb->query("SELECT id FROM ". $wpdb->prefix ."vtypes");		
			}
			...
			if($step == '3_5'){
				...
				if($query == 1){				
					$count = $wpdb->query("SELECT id FROM ". $wpdb->prefix ."vtypes_fstats");
				}	
			}

			if($step == '3_6'){
				$count = $wpdb->query("SELECT id FROM ". $wpdb->prefix ."bids");		
			}

			if($step == '3_7'){
				$query = $wpdb->query("CHECK TABLE ". $wpdb->prefix ."bcbroker_naps");
				if($query == 1){				
					$count = $wpdb->query("SELECT id FROM ". $wpdb->prefix ."bcbroker_naps");
				}	
			}	
	...
	if(current_user_can('administrator')){
		...
		if($step == '3_2'){	 /*****************/
			...
			foreach($places as $place => $title){
				if($place != 'success'){
					$premiumbox->update_option('naps_ap_instruction',$place, 1);
				}
			}
		...
		if($step == '3_4'){	 /*****************/
			...
			foreach($datas as $data){
				...
				if($maxexip > 0){
					...
					update_naps_meta($data->id, 'naps_constraints', $naps_constraints);
				}
			} 					
