...
jQuery(function($){
	...
	$(document).on('click', '.js_amount', function(){
		...
		var par = $(this).parents('.js_wrap_error');
		par.find('input').val(amount).trigger('change');
	});	
	...
	if($error > 0){
		...
		if($obmen_id > 0){
			...
			if(isset($obmen->id)){
				...
				set_action_bidstatus_new(0, $obmen, $naps, $vd1, $vd2);
...
function set_action_bidstatus_new($place, $obmen, $naps, $vd1, $vd2){
global $wpdb, $premiumbox;
...
global $wpdb, $premiumbox;	
	...
	set_action_bidstatus_new(1, $obmen, $naps, $vd1, $vd2);
