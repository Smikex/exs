jQuery(function($) {		
	...
	$(document).on('click','.user_comment',function(){
		...
		var hei = Math.ceil(($(window).height() - $('#window_user_comment').height()) / 2);
		var left = Math.ceil(($(window).width() - $('#window_user_comment').width()) / 2);
		$('#window_user_comment').css({'top': hei, 'left': left});		
