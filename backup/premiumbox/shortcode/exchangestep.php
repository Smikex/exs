...
global $wpdb, $premiumbox;
	...
	if(isset($item->id)){
		...
		$check_rule = '<div class="checkbox"><input type="checkbox" id="check_rule_step" name="check" value="1" /> '. sprintf(__('I read and agree with <a href="%s" target="_blank">the terms and conditions</a>','pn'), $premiumbox->get_page('tos') ) .'</div>';
...
global $wpdb, $premiumbox;
	...
	if(isset($item->id) and !in_array($status, $not_status)){
		...
		$def_status = array(
			...
			'verify' => array(
				'title' => __('Order is on checking','pn'),
				'text' => __('Order is on checking','pn'),
			),
			'realpay' => array(
				'title' => __('Order is paid','pn'),
