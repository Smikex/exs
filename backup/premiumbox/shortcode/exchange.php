...
global $wpdb, $naps_id, $naps_data, $premiumbox;	
	...
	if($show_data['mode'] == 1 and $naps_id > 0){
		...
		$text = apply_filters('naps_instruction', $text, $naps, $vd1, $vd2, 'timeline');
		...
		$text = apply_filters('naps_instruction', $text, $naps, $vd1, $vd2, 'description');
		$text = pn_strip_text(ctv_ml($text));		
		$description = '';	
		if($text){			
		...
		if($summ1 < $min1){
			$summ1_error = 'error';
			$summ1_error_txt = __('min','pn').'.: <span class="js_amount">'. $min1 .'</span> '.$vtype1;													
		}
		if($summ1 > $max1 and is_numeric($max1)){
			$summ1_error = 'error';
			$summ1_error_txt = __('max','pn').'.: <span class="js_amount">'. $max1 .'</span> '.$vtype1;													
		}
		if($summ1c < 0){
			$summ1c_error = 'error';
		}
		if($summ2 < $min2){
			$summ2_error = 'error';
			$summ2_error_txt = __('min','pn').'.: <span class="js_amount">'. $min2 .'</span> '.$vtype2;													
		}
		if($summ2 > $max2 and is_numeric($max2)){
			$summ2_error = 'error';
			$summ2_error_txt = __('max','pn').'.: <span class="js_amount">'. $max2 .'</span> '.$vtype2;													
		}
		if($summ2c < 0){
			$summ2c_error = 'error';
		}	
		...
		$array = array(
			...
			'[other_filter]' => apply_filters('exchange_other_filter', ''),
			'[result]' => '<div class="ajax_post_bids_res"></div>',
			'[check]' => $check,
			'[submit]' => $submit,
			'[filters]' => apply_filters('exchange_step1',''),
			'[naps_field]' => get_napspole_wline($naps, 'exchange'),
		...
		$array = apply_filters('exchange_html_list', $array, $naps, $vd1, $vd2, $cdata);
		...
		$html = apply_filters('exchange_html', $html, $naps, $vd1, $vd2, $cdata);			
