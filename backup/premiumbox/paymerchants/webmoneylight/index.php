	...
	class paymerchant_webmoney_light extends AutoPayut_Premiumbox{
		...
		function __construct($file, $title)
		{
			$map = array(
				...
				'WMY_PURSE', 'WMG_PURSE', 'WMX_PURSE', 'WMK_PURSE',
			);
		...
		function paymerchant_action_bid($item, $place, $naps_data){
			...
			if($item_id){
				...
				if($au_filter['enable'] == 1){			
					...
					$site_purse = '';
					if($vtype == 'Z'){
						$site_purse = is_deffin($this->m_data,'WMZ_PURSE');
					} elseif($vtype == 'R'){
						$site_purse = is_deffin($this->m_data,'WMR_PURSE');
					} elseif($vtype == 'E'){
						$site_purse = is_deffin($this->m_data,'WME_PURSE');
					} elseif($vtype == 'U'){
						$site_purse = is_deffin($this->m_data,'WMU_PURSE');
					} elseif($vtype == 'B'){
						$site_purse = is_deffin($this->m_data,'WMB_PURSE');
					} elseif($vtype == 'Y'){
						$site_purse = is_deffin($this->m_data,'WMY_PURSE');
					} elseif($vtype == 'G'){
						$site_purse = is_deffin($this->m_data,'WMG_PURSE');
					} elseif($vtype == 'X'){
						$site_purse = is_deffin($this->m_data,'WMX_PURSE');
					} elseif($vtype == 'K'){
						$site_purse = is_deffin($this->m_data,'WMK_PURSE');
					} 
