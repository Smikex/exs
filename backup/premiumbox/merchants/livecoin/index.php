...
description: [en_US:]LiveCoin merchant[:en_US][ru_RU:]мерчант LiveCoin[:ru_RU]
version: 1.3
*/

if(!class_exists('merchant_livecoin')){
	class merchant_livecoin extends Merchant_Premiumbox{

		function __construct($file, $title)
		{
			$map = array(
				'LIVECOIN_KEY', 'LIVECOIN_SECRET', 
			);
		...
		function merchants_settingtext(){
			...
			if(
				is_deffin($this->m_data,'LIVECOIN_KEY')  
				and is_deffin($this->m_data,'LIVECOIN_SECRET') 
			){
				$text = '';
			}
				...
				if(isset($item->id)){
					...
					?>
						...
						<div class="zone center">
							...
							<p><?php _e('In order to pay an ID order','pn'); ?> <b><?php echo $item_id; ?></b>,<br /> <?php _e('enter coupon code valued','pn'); ?> <b><?php echo $sum; ?> LiveCoin <?php echo is_site_value($item->vtype1); ?></b>:</p>
			...
			if($hashed){
				...
				if(isset($item->id)){
					...
					if($status=='new' and $m_id and $m_id == $this->name){
						...
						if($code){
							try{
								$res = new LiveCoin(is_deffin($this->m_data,'LIVECOIN_KEY'),is_deffin($this->m_data,'LIVECOIN_SECRET'));
...
new merchant_livecoin(__FILE__, 'LiveCoin');