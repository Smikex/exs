<?php
/*
title: [en_US:]BTC-UP[:en_US][ru_RU:]BTC-UP[:ru_RU]
description: [en_US:]BTC-UP merchant[:en_US][ru_RU:]мерчант BTC-UP[:ru_RU]
version: 1.3
*/

if(!class_exists('merchant_btcup')){
	class merchant_btcup extends Merchant_Premiumbox{

		function __construct($file, $title)
		{
			$map = array(
				'KEY_COUPON','SECRET_COUPON', 
			);
		...
		function merchants_settingtext(){
			...
			if(
				is_deffin($this->m_data,'KEY_COUPON') 
				and is_deffin($this->m_data,'SECRET_COUPON')
			){
				$text = '';
			}
				...
				if(isset($item->id)){
					...
					?>
						...
						<div class="zone center">
							...
							<p><?php _e('In order to pay an ID order','pn'); ?> <b><?php echo $item_id; ?></b>,<br /> <?php _e('enter coupon code valued','pn'); ?> <b><?php echo $sum; ?> BTC-UP <?php echo is_site_value($item->vtype1); ?></b>:</p>
				...
				if(isset($item->id)){
					...
					if($status=='new' and $m_id and $m_id == $this->name){
						...
						if($code){
							try{
								$res = new BTCUP();
								$info = $res->redeem_voucher($code, is_deffin($this->m_data,'KEY_COUPON'), is_deffin($this->m_data,'SECRET_COUPON'));
								if($info){
									$merch_sum = is_isset($info,'couponAmount');
									$merch_currency = strtoupper(is_isset($info,'couponCurrency'));
									$merch_trans_id = trim(is_isset($info,'transID'));
...
new merchant_btcup(__FILE__, 'BTCUP');