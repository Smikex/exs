	...
	class merchant_blockio extends Merchant_Premiumbox {
		...
		function summ_to_pay($sum, $m_id ,$item, $naps){ 
 		...
 		function merchants_action_bid($temp, $pay_sum, $item, $naps){
			...
			$currency = pn_strip_input($item->vtype1);	
			
			$enable_currency = array('BTC','LTC','DOGE');
			
			if(in_array($currency, $enable_currency)){
								...
								foreach($res->data->txs as $data){
									...
									if($amount > 0 and $address){
										...
										if(isset($item->id)){
											...
											if($amount >= $sum or $invalid_minsum == 1){
													
												do_action('merchant_confirm_count', $item->id, $confirmations, $item, '', $n_conf, $this->name);	
		...
		function merchant_formstep_after($temp, $m_id, $item, $naps){
			if($m_id and $m_id == $this->name){
				$temp .= '
														...
														if($amount > 0 and $address){
															if($naschet == $address){	
																...
																if($amount >= $sum or $invalid_minsum == 1){
																	...
																	do_action('merchant_confirm_count', $item->id, $confirmations, $item, $naps, $n_conf, $this->name);	
