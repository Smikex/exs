...
$prefix = $wpdb->prefix; 
	...
	$query = $wpdb->query("SHOW COLUMNS FROM ".$wpdb->prefix ."bids LIKE 'pay_ac'");
    if ($query == 0) { 
		$wpdb->query("ALTER TABLE ".$wpdb->prefix ."bids ADD `pay_ac` varchar(250) NOT NULL");
    }	
	$query = $wpdb->query("SHOW COLUMNS FROM ".$wpdb->prefix ."bids LIKE 'pay_sum'");
    if ($query == 0) { 
		$wpdb->query("ALTER TABLE ".$wpdb->prefix ."bids ADD `pay_sum` varchar(50) NOT NULL default '0'");
    }		
	/* end bids */
