...
class trev_discount_List_Table extends WP_List_Table {
    ...
    function column_default($item, $column_name){
		...
		if($column_name == 'discount'){
		    ...
		    return is_my_money($item->discount) . '%';	
    ...
    function column_title($item){
		...
		$primary = apply_filters('discount_manage_ap_primary', '>' . is_my_money($item->sumec), $item);
    ...
    function prepare_items() {
		...
		$total_items = $wpdb->query("SELECT id FROM ". $wpdb->prefix ."user_discounts WHERE id > 0 $where");
