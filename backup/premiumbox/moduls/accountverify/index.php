...
global $wpdb;
	...
	foreach($items as $item){
		...
		if($result){
			do_action('pn_user_accounts_delete_after', $item_id, $item);
		}
	}
	
}

add_action('pn_user_accounts_delete_after', 'pn_user_accounts_delete_accountverify', 10, 2);
function pn_user_accounts_delete_accountverify($item_id, $item){
	$usac_id = $item->usac_id;
	delete_userwallets_files($usac_id);
}

add_action('pn_valuts_delete_after', 'pn_valuts_delete_accountverify');
