...
description: [ru_RU:]!Не активируйте без необходимости! Отображение кода купона авто выплаты в карточке заявки[:ru_RU][en_US:]!Do not activate it without any reason! Show automatic payout coupon code in request form.[:en_US]
version: 1.1
...
global $wpdb;	
	...
	$query = $wpdb->query("SHOW COLUMNS FROM ".$wpdb->prefix ."bids LIKE 'btc_code_info'");
    if ($query == 0){
		$wpdb->query("ALTER TABLE ".$wpdb->prefix ."bids ADD `btc_code_info` varchar(250) NOT NULL");
    }	
