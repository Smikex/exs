...
description: [ru_RU:]Логирование изменения курсов обмена. Окно уведомления[:ru_RU][en_US:]Logging changes in exchange rates. Notification window[:en_US]
version: 1.1
	...
	if(isset($naps->id)){
		...
		$lcurs2 = is_my_money($naps->curs2);
 		if($lcurs1 != $curs1 or $lcurs2 != $curs2){
			...
			$arr['naps_id'] = intval($naps->id);
			$arr['user_id'] = intval($user_id);
			$arr['user_login'] = is_isset($ui,'user_login');
			$arr['v1'] = intval($naps->valut_id1);
			$arr['v2'] = intval($naps->valut_id2);
			$arr['lcurs1'] = is_my_money($lcurs1);
			$arr['lcurs2'] = is_my_money($lcurs2);
