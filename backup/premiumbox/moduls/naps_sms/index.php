...
add_filter('bid_instruction_tags', 'napssms_bid_instruction_tags', 1000, 5);
function napssms_bid_instruction_tags($instruction, $item, $naps, $vd1, $vd2){
global $premiumbox;
	...
	if($hideinstruction == 1){
		...
		if($new_user_option == 0 or isset($item->new_user) and $item->new_user == 1 and $new_user_option == 1){
			$naps_id = $naps->id;
			$bid_id = $item->id;
			$button = intval(get_naps_meta($naps_id, 'sms_button'));
...
add_filter('merchant_formstep_after','napssms_merchant_formstep_after', 9999, 4);
