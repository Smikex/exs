	...
	foreach($naps as $nap){
		...
		if(isset($data->id)){
			...
			if($what == 0){
				...
				$c2 = $data->curs2;
				$one_pers1 = $curs1 / 100;
				$ncurs1 = $curs1 + ($one_pers1 * $c1);
				$one_pers2 = $curs2 / 100;
				$ncurs2 = $curs2 + ($one_pers2 * $c2);
			}
			$arr['curs1'] = $ncurs1;
			$arr['curs2'] = $ncurs2;
			if($ncurs1 > 0 and $ncurs2 > 0){
				$wpdb->update($wpdb->prefix ."naps", $arr, array('id'=> $naps_id));
				do_action('naps_change_course', $naps_id, $nap, $ncurs1, $ncurs2, 'reservcurs');
			}
		}		
	}
...
add_action('admin_menu', 'pn_adminpage_reservcurs');
function pn_adminpage_reservcurs(){
global $premiumbox;	
...
add_action('pn_adminpage_title_pn_reservcurs', 'pn_admin_title_pn_reservcurs');
function pn_admin_title_pn_reservcurs($page){
