...
global $wpdb, $bids_data;
	...
	if(isset($bids_data->id)){
		...
		if(!$currency_id){ $currency_id = $bids_data->valut1i; } 
		
		$display = intval(is_isset($n_atts,'display'));
		if(!$display){ $display = intval(is_isset($n_atts,'vid')); } /* remove with 1.6 */
		
		$id_bid = intval(is_isset($bids_data,'id'));
		$naschet = pn_maxf_mb(pn_strip_input(is_isset($bids_data,'naschet')),500);
		$sum = is_my_money(is_isset($bids_data,'summ1_dc'));
	
		$theval = get_now_vaccount($currency_id, $display, $id_bid, $sum, $naschet, array());
		if(!$hide){
			if($theval){
				$new_bid_data = $wpdb->get_row("SELECT * FROM ". $wpdb->prefix ."bids WHERE id='$id_bid'");
				$hashdata = @unserialize($new_bid_data->hashdata);
				if(!is_array($hashdata)){ $hashdata = array(); }
				if(!is_pn_crypt($hashdata['naschet'], $theval)){
