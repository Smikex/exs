	...
	if($dej == 1){
		...
		if(isset($data->id)){
			if($what == 0){
				...
				$c2 = $data->curs2;
				$one_pers1 = $curs1 / 100;
				$cdata['curs1'] = $curs1 + ($one_pers1 * $c1);
				$one_pers2 = $curs2 / 100;
				$cdata['curs2'] = $curs2 + ($one_pers2 * $c2);
			}
		}		
	}
...
global $premiumbox;	
	...
	add_submenu_page("pn_moduls", __('Rate is depends on exchange amount','pn'), __('Rate is depends on exchange amount','pn'), 'administrator', "pn_sumcurs", array($premiumbox, 'admin_temp'));
}
