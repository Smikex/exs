...
?>	
<style>
.column-parser{ width: 260px!important; }
</style>
...
function parser_naps_manage_ap_col($show, $column_name, $item){
	if($column_name == 'parser'){
		...
		$html .= '
		<select name="parser['. $item->id .']" autocomplete="off" id="naps_parser_'. $item->id .'" class="naps_parser" style="width: 260px; display: block; margin: 0 0 10px;"> 
		';
			$enable = 0;
		...
		<div id="the_naps_parser_'. $item->id .'" '. $style .'>
			<input type="text" name="nums1['. $item->id .']" style="width: 100px; float: left; margin: 2px 5px 0 0;" value="'. pn_strip_input($item->nums1) .'" />
			<div style="float: left; margin: 5px 10px 0 10px;">=></div>
			<input type="text" name="nums2['. $item->id .']" style="width: 100px; float: left; margin: 2px 5px 0 0;" value="'. pn_strip_input($item->nums2) .'" />				
...
?>
	...
	?>
		...
		<tr>
			...
			<td>
				<div class="premium_wrap_standart">
					<input type="text" name="nums1" value="<?php echo pn_strip_input(is_isset($data, 'nums1'));?>" />										
				</div>			
			</td>
			<td>
				<div class="premium_wrap_standart">
					<input type="text" name="nums2" value="<?php echo pn_strip_input(is_isset($data, 'nums2'));?>" />											
				</div>	
			</td>
		</tr>
<script type="text/javascript">
...
function parser_bcparser_def_course($darr, $item, $options){
	...
	if($parser > 0){
		...
		$ncurs1 = plus_persent_curs($curs1, $nums1);
		$ncurs2 = plus_persent_curs($curs2, $nums2);				
		if($ncurs1 > 0){
			$darr = array(
				'course1' => $ncurs1,
				'course2' => $ncurs2,
