...
/*
title: [ru_RU:]Дополнительная комиссия зависящая от суммы обмена[:ru_RU][en_US:]Additional fee that depends on exchanged amount[:en_US]
description: [ru_RU:]Дополнительная комиссия зависящая от суммы обмена[:ru_RU][en_US:]Additional fee that depends on exchanged amount[:en_US]
version: 1.1
...
/* BD */
add_action('pn_moduls_active_'.$name, 'bd_pn_moduls_active_dopsumcomis');
function bd_pn_moduls_active_dopsumcomis(){
global $wpdb;	
...
add_action('pn_naps_delete', 'pn_naps_delete_dopsumcomis');
function pn_naps_delete_dopsumcomis($item_id){
global $wpdb;	
	$wpdb->query("DELETE FROM ".$wpdb->prefix."naps_dopsumcomis WHERE naps_id = '$item_id'");
}

add_action('pn_naps_copy', 'pn_naps_copy_dopsumcomis', 1, 2);
function pn_naps_copy_dopsumcomis($last_id, $new_id){
global $wpdb;
...
function get_dopsumcomis_html_line($item){
	...
	$temp = '
	<div class="napscurs_line js_dopsumcomis_line" data-id="'. $item_id .'">
		<div class="napscurs_item">
			<div class="napscurs_title">
				'. __('Amount','pn') .'
			</div>
			<div class="napscurs_input">
				<input type="text" name="" style="width: 100px;" class="rate_sum" value="'. is_my_money(is_isset($item,'sum_val')) .'" />
			</div>
		</div>';
		
		if($item_id){
			$temp .= '<div class="napscurs_add js_dopsumcomis_add">'. __('Save','pn') .'</div>';
			$temp .= '<div class="napscurs_del js_dopsumcomis_del">'. __('Delete','pn') .'</div>';
		} else {
			$temp .= '<div class="napscurs_add js_dopsumcomis_add">'. __('Add new','pn') .'</div>';
		}
		
		$temp .= '
			<div class="premium_clear"></div>	

		<div class="napscurs_line_title">
		...
		$temp .= '
			<div class="napscurs_item">
				<div class="napscurs_input">
					<input type="text" name="" style="width: 100px;" class="rate_j1" value="'. is_my_money(is_isset($item,'com_box_summ1')) .'" /> S
				</div>
			</div>
			<div class="napscurs_item">
				<div class="napscurs_input">
					<input type="text" name="" style="width: 100px;" class="rate_j2" value="'. is_my_money(is_isset($item,'com_box_pers1')) .'" /> %
				</div>
			</div>
		';
		...
		$temp .= '
			<div class="napscurs_item">
				<div class="napscurs_input">
					<input type="text" name="" style="width: 100px;" class="rate_j3" value="'. is_my_money(is_isset($item,'com_box_summ2')) .'" /> S
				</div>
			</div>
			<div class="napscurs_item">
				<div class="napscurs_input">
					<input type="text" name="" style="width: 100px;" class="rate_j4" value="'. is_my_money(is_isset($item,'com_box_pers2')) .'" /> %
				</div>
			</div>
		';
...
global $wpdb;
	...
	if(current_user_can('administrator') or current_user_can('pn_naps')){
		...
		$j1 = is_my_money(is_param_post('j1'));
		$j2 = is_my_money(is_param_post('j2'));
		$j3 = is_my_money(is_param_post('j3'));
		$j4 = is_my_money(is_param_post('j4'));
		if($data_id > 0){
...
add_action('tab_naps_tab5', 'tab_naps_tab5_dopsumcomis', 20);
function tab_naps_tab5_dopsumcomis($data){	
	if(isset($data->id)){ 
		$data_id = $data->id;
	?>
		<tr>
			<th><?php _e('Additional fee that depends on exchanged amount','pn'); ?></th>
			<td colspan="2">
		...
		<tr>
			<td></td>
			<td colspan="2">
				<?php pn_help(__('More info','pn'), __('Specify the value of the additional fee that depends on the "Send" amount.','pn')); ?>
			</td>
		</tr>
...
$(function(){
	...
	$(document).on('click', '.js_dopsumcomis_add', function(){ 
		...
		var sum = parseFloat(par.find('.rate_sum').val());
		var j1 = parseFloat(par.find('.rate_j1').val());
		var j2 = parseFloat(par.find('.rate_j2').val());
		var j3 = parseFloat(par.find('.rate_j3').val());
		var j4 = parseFloat(par.find('.rate_j4').val());
		...
		$.ajax({
			type: "POST",
			url: "<?php pn_the_link_post('dopsumcomis_add');?>",
			dataType: 'json',
			data: param,
	...
	$(document).on('click', '.js_dopsumcomis_del', function(){
		...
		$.ajax({
			type: "POST",
			url: "<?php pn_the_link_post('dopsumcomis_del');?>",
			dataType: 'json',
			data: param,
			error: function(res, res2, res3){
				<?php do_action('pn_js_error_response', 'ajax'); ?>
...
/* filter */
add_filter('set_calc_data', 'set_calc_data_dopsumcomis', 0, 9);
function set_calc_data_dopsumcomis($cdata, $vd1, $vd2, $naps, $user_id, $post_sum, $check1, $check2, $dej){
global $wpdb;
	
	$naps_id = $naps->id;
	$post_sum = is_my_money($post_sum);
	if($dej == 1){
		$cc = $wpdb->query("SELECT id FROM ". $wpdb->prefix ."naps_dopsumcomis WHERE naps_id='$naps_id'");
		if($cc > 0){
			$cdata['dis1c'] = 1;
			$cdata['dis2'] = 1;	
			$cdata['dis2c'] = 1;
		}
		$data = $wpdb->get_row("SELECT * FROM ". $wpdb->prefix ."naps_dopsumcomis WHERE naps_id='$naps_id' AND ('$post_sum' -0.0) >= sum_val ORDER BY (sum_val -0.0) DESC");
		if(isset($data->id)){
			$cdata['com_box_summ1'] = $data->com_box_summ1;
			$cdata['com_box_pers1'] = $data->com_box_pers1;		
			$cdata['com_box_summ2'] = $data->com_box_summ2;
