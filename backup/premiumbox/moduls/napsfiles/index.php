...
global $wpdb, $premiumbox;	
	...
	$options = array('txt','xml','fromfee','tofee');			
				...
				if($output == 1){
					...
					if(isset($v[$valid1]) and isset($v[$valid2])){
						...
						if($fromfee_setting == 1){
							if($ob->com_summ1){ 
								$fromfee[] = is_my_money($ob->com_summ1,$num) . ' '. pn_strip_input($vd1->vtype_title);
							}
							if($ob->com_pers1){
								$fromfee[] = is_my_money($ob->com_pers1,$num).' %';
							}							
						} elseif($fromfee_setting == 2){
							if($ob->com_summ1_check){ 
								$fromfee[] = is_my_money($ob->com_summ1_check,$num) . ' '. pn_strip_input($vd1->vtype_title);
							}
							if($ob->com_pers1_check){
								$fromfee[] = is_my_money($ob->com_pers1_check,$num).' %';
							}							
						} else {
							if($ob->com_box_summ1){ 
						...
						if($tofee_setting == 1){
							if($ob->com_summ2){ 
								$tofee[] = is_my_money($ob->com_summ2,$num) . ' '. pn_strip_input($vd2->vtype_title);
							}
							if($ob->com_pers2){
								$tofee[] = is_my_money($ob->com_pers2,$num).' %';
							}							
						} elseif($tofee_setting == 2){
							if($ob->com_summ2_check){ 
								$tofee[] = is_my_money($ob->com_summ2_check,$num) . ' '. pn_strip_input($vd2->vtype_title);
							}
							if($ob->com_pers2_check){
								$tofee[] = is_my_money($ob->com_pers2_check,$num).' %';
							}							
						} else {
							if($ob->com_box_summ2){ 
						...
						if($ob->maxsumm1 > 0){
							$lines['maxamount'] = is_my_money($ob->maxsumm1,$num).' '.pn_strip_input($vd1->vtype_title);
						}
						$params = array();
