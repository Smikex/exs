...
global $wpdb, $premiumbox;	
	...
	$naps2_data = get_table2_naps_filter($naps2, $val_types2, $standart2);	
				$hidecurrtype = intval($premiumbox->get_option('exchange','hidecurrtype'));		
				if($hidecurrtype == 0){
...
// левая колонка
					...
					<div class="row">
					<div class="col-md-4">
					<div class="electronic_wallets">
						<h4 class="main_title">
							1. Отдаете
						</h4>
						<h5 class="title">Валюта отдачи:</h5>
						...
						<div class="xtp_left_col_table">
							';
							$exchange_table2_head .= apply_filters('exchange_table2_part', '',  __('You send','pn'), $naps1_data, 'left');
	...
	// Правая колонка						
							...
							<div class="electronic_wallets">
								...
								<div class="numb">
									<input class="sum" type="text" value="746.68" disabled="disabled">
									<img class="ico" src="images/advcash-ico.png" alt="advcash">
								</div>
								<h5 class="title">Валюта отдачи:</h5>
								<div class="radio-btns">
						...
						<div class="xtp_right_col_table">	';
							...
							$exchange_table2_head .= apply_filters('exchange_table2_part', '', __('You receive','pn'), $naps2_data, 'right');	



							$exchange_table2_head .= '<div class="col-md-4">
							<div class="electronic_wallets">
								<h4 class="main_title">
									3. Ввод данных
								</h4>';
...
global $wpdb, $premiumbox;	
	...
	if(isset($naps->id)){
		...
		if($premiumbox->get_option('exchange','flysum') == 1){
			...
			if($summ1 < $min1){
				$summ1_error = 'error';
				$summ1_error_text = __('min','pn').'.: <span class="js_amount">'. $min1 .'</span> '.$vtype1;													
			}
									
			if($summ1 > $max1 and is_numeric($max1)){
				$summ1_error = 'error';
				$summ1_error_text = __('max','pn').'.: <span class="js_amount">'. $max1 .'</span> '.$vtype1;													
			}
									
			if($summ2 < $min2){
				$summ2_error = 'error';
				$summ2_error_text = __('min','pn').'.: <span class="js_amount">'. $min2 .'</span> '.$vtype2;													
			}
									
			if($summ2 > $max2 and is_numeric($max2)){
				$summ2_error = 'error';
				$summ2_error_text = __('max','pn').'.: <span class="js_amount">'. $max2 .'</span> '.$vtype2;													
			}								
		...
		';
					...
					$temp .= '	
						<div class="js_error js_summ1_error">'. $summ1_error_text .'</div>
					</div>';
				...
				<div class="xtp_commis_wrap js_wrap_error js_wrap_error_br '. $summ1c_error .'" '. $viv_com1 .'>';
					...
					<div class="js_error js_summ1c_error">'. $summ1c_error_text .'</div>
					<div class="clear"></div>
				</div>';
			...
			<div class="xtp_right_col_table">
				<div class="xtp_curs_wrap">';
					...
					$temp .= '
						<div class="js_error js_summ2_error">'. $summ2_error_text .'</div>
					</div>';
				...
				<div class="xtp_commis_wrap js_wrap_error js_wrap_error_br '. $summ2c_error .'" '. $viv_com2 .'>';
					...
					$temp .= '
					<div class="xtp_commis_text">'. __('With fees','pn') .'</div>
					<div class="js_error js_summ2c_error">'. $summ2c_error_text .'</div>
					<div class="clear"></div>
				</div>';				
